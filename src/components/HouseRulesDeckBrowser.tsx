...
export function HouseRulesDeckBrowser() {
  ...
  return (
    <div className="rounded-2xl border border-seam bg-surface p-4">
      <h3 className="mb-1 text-xs font-semibold uppercase tracking-[0.2em] text-ink-dim">
        The Deck
      </h3>
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="shrink-0 rounded-full border border-seam px-2 py-0.5 text-[10px] uppercase tracking-wide text-ink-dim">
                  {ENFORCEMENT_LABEL[rule.enforcement]}
                </span>
              </div>
              <p className="text-xs text-accent">{rule.tagline}</p>
              {isOpen && <p className="mt-1 text-xs text-ink-dim">{rule.description}</p>}
            </button>
          );
        })}
      </div>
