...
export async function getWeekGames(season: number, week: number): Promise<Game[]> {
  ...
  const { data } = await supabase
    .from("nfl_games")
    .select(
      "id, season, week, season_type, kickoff_at, home_team, away_team, home_name, away_name, home_score, away_score, status, status_detail, network, venue, neutral_site, time_valid",
    )
    .eq("season", season)
    .eq("week", week)
    .order("kickoff_at", { ascending: true });
...
// Bucketed by the Eastern date, not the UTC one — see nfl-date.ts.
